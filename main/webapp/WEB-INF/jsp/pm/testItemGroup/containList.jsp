...
<body>
            <h3>组合已包含项目</h3>
            <p align="right">记录总行数：<label id="containListNumber">${fn:length(containList)}</label></p>
            <table id="containList">
                <tr>
                	<th style="width: 30px;" class="quan"><a href="javascript:void(0)" class="not"></a></th>
                ...
                <c:if test="${containList != null || fn:length(containList) != 0}">
	                ...
	                <tr id="tr_contain_${containList.id }">
	                    ...
	                    <td title="<c:if test="${containList.sexId == 3}">不限</c:if><c:if test="${containList.sexId == 1}">男</c:if><c:if test="${containList.sexId == 2}">女</c:if>">
	                    	<c:if test="${containList.sexId == 3}">不限</c:if><c:if test="${containList.sexId == 1}">男</c:if><c:if test="${containList.sexId == 2}">女</c:if>
	                    </td>
	                    <td>${containList.sexId}</td>
	                    <td>${containList.id}</td>
	                </tr>
	                </c:forEach>
                </c:if>
            </table>

			...
			<div class="dels">
				<table id="listTable">
					...
					<c:if test="${resultList == null || fn:length(resultList) == 0}">
			        	<tr><td colspan="11"><span style="color: red">未查询到数据</span></td></tr>
			        </c:if>
					<c:forEach items="${resultList}" var="dictCode" varStatus="status">
						<tr id="tr_${dictCode.id}">
							...
							<td title="${dictCode.name}"><c:out value="${dictCode.name}" escapeXml="true" /></td>
							<td title="${dictCode.enShortName}"><c:out value="${dictCode.enShortName}" escapeXml="true" /></td>
							<td title="${dictCode.enName}"><c:out value="${dictCode.enName}" escapeXml="true" /></td>
							<td title="${dictCode.whonetCode}"><c:out value="${dictCode.whonetCode}" escapeXml="true" /></td>
							<td title="${dictCode.fastCode}"><c:out value="${dictCode.fastCode}" escapeXml="true" /></td>
							<td title="${dictCode.displayOrder}"><c:out value="${dictCode.displayOrder}" escapeXml="true" /></td>
							<td title="${dictCode.memo}"><c:out value="${dictCode.memo}" escapeXml="true" /></td>
							<td class="as">

...
<script src="${ctx}/js/org/centerOrg.js?var=1.0.0.4"></script>
<script src="${ctx}/js/enterToTab.js"></script>
<title>独立实验室维护修改</title>
...
<body class="bg">
    ...
    <form id="editForm" name="editForm">
		...
		<div>
		   ...
		   <span>英文简称</span><input onblur="checkSpecialSymbol('enShortName',<%=RE_SSYMBOL%>,'<%=MSG_SSYMBOL%>')" type="text" id="enShortName" name="enShortName" maxlength="45" value="<c:out value='${entity.enShortName}' escapeXml='true'/>"/>
		</div>
		<div>
		  ...
		  <span><i>*</i>网站名称（地址）</span><input onblur="checkSpecialSymbol('webUrl',<%=RE_SSYMBOL%>,'<%=MSG_SSYMBOL%>','<%=COMFIRM_ID%>','<%=CANCEL_ID%>')" type="text" id="webUrl" name="webUrl" maxlength="55" value="<c:out value='${entity.webUrl}' escapeXml='true'/>"/>
		</div>
		<div>
		    ...
		    <span><i>*</i>联系电话</span><input onblur="checkSpecialSymbol('telephone',<%=RE_SSYMBOL%>,'<%=MSG_SSYMBOL%>','<%=COMFIRM_ID%>','<%=CANCEL_ID%>')" type="text" id="telephone" name="telephone" maxlength="50" value="<c:out value='${entity.telephone}' escapeXml='true'/>"/>
		</div>
		<div>
